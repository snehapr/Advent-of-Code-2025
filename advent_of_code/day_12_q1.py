...
with open(os.path.join(os.path.dirname(__file__), "presents_main.txt"), "r", encoding="utf8") as f:
    data = f.read().replace('\r', '')

# Split into sections, remove empty, and process
sections = [x for x in data.split('\n\n') if x.strip()]
regions_raw = sections.pop().split('\n')
regions = []
for line in regions_raw:
    if line.strip():
        ...
        regions.append({'counts': counts, 'width': width, 'height': height})

# Each present is the lines after the first line in each section, joined together
presents = [''.join(section.split('\n')[1:]) for section in sections]
...
for region in regions:
    ...
    for i, present in enumerate(presents):
        # Count number of '#' in present and multiply by counts[i]
        size += present.count('#') * counts[i]
